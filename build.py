...
Usage:
    ...
    python build.py test-lexer
    python build.py test-parser
...
class Colors:
    ...
    def __init__(self):
        ...
        self.supported = (
            ...
            'ANSI' in os.environ.get('TERM_PROGRAM', '')
        )
...
class HLangBuilder:
    ...
    def find_python(self):
        ...
        for cmd in candidates:
            try:
                result = self.run_command([cmd, "--version"], capture_output=True, check=False)
        ...
        if platform.system() == "Windows":
            try:
                result = self.run_command([f"py", f"-{self.python_version}", "--version"], capture_output=True, check=False)
    ...
    def show_help(self):
        ...
        print(self.colors.yellow("  python build.py setup     - Install dependencies and set up environment"))
        print(self.colors.yellow("  python build.py build     - Compile ANTLR grammar files"))
        print(self.colors.yellow("  python build.py check     - Check if required tools are installed"))
        print()
        print(self.colors.green("Testing:"))
        print(self.colors.yellow("  python build.py test-lexer  - Run lexer tests and generate reports"))
        print(self.colors.yellow("  python build.py test-parser - Run parser tests and generate reports"))
        print()
        print(self.colors.green("Cleaning:"))
        print(self.colors.yellow("  python build.py clean         - Clean build and external directories"))
        print(self.colors.yellow("  python build.py clean-cache   - Clean Python cache files"))
        print(self.colors.yellow("  python build.py clean-reports - Clean test reports directory"))
        print(self.colors.yellow("  python build.py clean-venv    - Remove virtual environment"))
        print()
        ...
        print()
        print(self.colors.blue("Quick start: python build.py setup && python build.py build"))
    ...
    def check_dependencies(self):
        ...
        if self.command_exists("java"):
            ...
            if platform.system() == "Windows":
                print(self.colors.yellow("    - Download from https://adoptium.net/ or https://www.oracle.com/java/technologies/downloads/"))
                print(self.colors.yellow("    - Or use Chocolatey: choco install openjdk"))
                print(self.colors.yellow("    - Or use Scoop: scoop install openjdk"))
            elif platform.system() == "Darwin":
                print(self.colors.yellow("    - On macOS: brew install openjdk"))
            else:
                print(self.colors.yellow("    - On Ubuntu/Debian: sudo apt install openjdk-17-jdk"))
                print(self.colors.yellow("    - Or download from https://adoptium.net/"))
        ...
        if python_cmd:
            ...
            print(self.colors.yellow(f"  Please install Python {self.python_version} manually:"))
            print(self.colors.yellow("    - Download from https://www.python.org/downloads/"))
            if platform.system() == "Windows":
                print(self.colors.yellow(f"    - Or use Chocolatey: choco install python --version={self.python_version}"))
                print(self.colors.yellow("    - Or use Scoop: scoop install python"))
            elif platform.system() == "Darwin":
                print(self.colors.yellow(f"    - On macOS: brew install python@{self.python_version}"))
            else:
                print(self.colors.yellow(f"    - On Ubuntu/Debian: sudo apt install python{self.python_version} python{self.python_version}-venv python3-pip"))
    ...
    def setup_environment(self):
        ...
        print(self.colors.yellow("Installing Python dependencies in virtual environment..."))
        self.run_command([str(self.venv_pip), "install", "-r", "requirements.txt"])
        print(self.colors.green("Python dependencies installed in virtual environment."))
        
        print(self.colors.green(f"Setup completed! Virtual environment is ready at {self.venv_dir}"))
    ...
    def build_grammar(self):
        ...
        (self.build_dir / "src" / "grammar").mkdir(exist_ok=True)
        ...
        (self.build_dir / "src" / "__init__.py").touch()
        (self.build_dir / "src" / "grammar" / "__init__.py").touch()
        
        # Copy Python files
        print(self.colors.yellow("Copying Python files from src/grammar/ to build/src/grammar/"))
        lexererr_src = self.root_dir / "src" / "grammar" / "lexererr.py"
        lexererr_dst = self.build_dir / "src" / "grammar" / "lexererr.py"
    ...
    def clean_all(self):
        ...
        print(self.colors.yellow("Cleaning build and external directories..."))
        ...
        print(self.colors.green("Cleaned build and external directories."))
        self.clean_cache()
    ...
    def test_lexer(self):
        ...
        print(self.colors.green(f"Lexer tests completed. Reports generated at {lexer_report_dir}/index.html"))
        self.clean_cache()
    ...
    def test_parser(self):
        ...
        print(self.colors.green(f"Parser tests completed. Reports generated at {parser_report_dir}/index.html"))
        self.clean_cache()
